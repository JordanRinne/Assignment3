...
# user schema:
# {user_name: {'bio':{'entry':, 'timestamp':},
# 'posts':[{'entry':, 'timestamp':}]} }
# post schema
                ...
                try:
                    ...
                    if 'join' in command:
                        ...
                        if len(command) != 1:
                            ...
                            status = "error"
                            message = "Extra fields provided to"
                            " join command object."
                        elif not all(
                            ...
                            if not fetched_user:
                                message = 'Welcome to ICS32 Distributed Social'
                                f', {uname}!'
                                status = 'ok'
    ...
    def _create_post(self, username, entry, timestamp):
        ...
        with posts_file_lock:
            ...
            existing_posts.insert(
                0,
                {'user': username, 'entry': entry, 'timestamp': timestamp}
            )
